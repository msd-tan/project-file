...
async def get_csv_data():
    ...
    if response.status_code == 200:
        logging.info("preparing to Download CSV data...")
        csv_text = response.content.decode('shift-jis')
...
async def download_csv_data():
    ...
    if response.status_code == 200:
        ...
        csv_response.headers["Content-Disposition"] = "attachment; filename=data555.csv"
        return csv_response
    else:
        raise HTTPException(status_code=response.status_code, detail=response.text)
...
if __name__ == '__main__':
    uvicorn.run(app='TestCSVAPI11:app', host="127.0.0.1", port=8513, reload=True)
